...
class Get_imagev2():
    def __init__(self):
        ...
        rospy.Subscriber("/zafterimage", Image, self.depth_callback)
    ...
    def depth_callback(self, data):
        try:
            if(self.display_mode == 'depth'):
                ...
                tmp = self.bridge.imgmsg_to_cv2(data, "mono8")    # for depth image 16UC1
                #print(tmp)
                #print(tmp.shape)
                #print(type(tmp))
                #print(data[10][10])
                ...
                #tmp=cv2.dilate(tmp,kernel,iterations = 4)
