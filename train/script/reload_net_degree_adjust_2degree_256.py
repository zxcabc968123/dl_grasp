...
net_path = '/home/allen/dl_grasp/src/train/Save_net/14object/drop/1121_14object_dropoutv6'
...
def create_photo_array(data1):
    ...
    photo_array = cv2.resize(photo_array, (256,256), interpolation=cv2.INTER_CUBIC)
    for i in range(len(data1)-1):
        image=cv2.imread(data1[i+1],0)
        image = cv2.resize(image, (256,256), interpolation=cv2.INTER_CUBIC)
        photo_array=np.concatenate((photo_array,image))
    photo_array=photo_array.reshape((-1,256,256,1))
    ########normalize
...
def get_test_image(image_path,reload_sm_keras):
    ...
    ########
    result_point[0][0]=result_point[0][0]*640
    result_point[0][1]=result_point[0][1]*480
...
def main():
    ...
    for i in range(len(validation_input)):
        temp_input=validation_input[i].reshape(-1,256,256,1)
        ##########predict time#######
        t0 = time.time()
